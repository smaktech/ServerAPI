...
const Review = require("../../models/createStdEvalAnswer");



router.get("/getevaluationlist",  async (req, res) => {
  
  const evaluationData = await Evaluation.find().exec();
  // console.log('This is evaluiation data ', evaluationData);
...
router.get("/getevaluationrevlist",  async (req, res) => {

  const evaluationData = await Review.find().exec();
  // console.log('This is getevaluationrevlist data ', evaluationData);
...
router.get("/getEvaluation",  async (req, res,  ) => {
  ...
  const newevaluationData = await Evaluation.findOne({
    _id: req.query.id}).exec();
  // console.log('New ............ Evaluation Data', newevaluationData, );
  // console.log('Request....... evaluationData', req.query.id);


  res.status(200).json({
    status: true,
    results: newevaluationData,
  });
});
