...
router.get("/filterCourse", async (req, res) => {
  ...
  const { boardID, subBoardID, classesID, subjectID } = req.query;
  ...
  const documentsCount = await Course.countDocuments(query).exec();
  ...
  try {
    const courses = await Course.find(query)
      .populate({
        path: "boardID subBoardID classesID subjectID",
      })
      .limit(limit)
      .skip(startIndex)
...
router.get("/getCourseById/:courseID", async (req, res) => {
  try {
    const courses = await Course.findOne({ _id: req.params.courseID }).populate(
      [
        { path: "subBoardID" },
        { path: "classesID" },
        { path: "subjectID" },
