...
router.get("/getAnswer",  async (req, res,  ) => {
  ...
  // answerData.map((row, index) => 
  // ModifiedData[row.ID] = row
  // )
...
router.get("/answer/createAnswer", async (req, res) => {
  ...
  const documentsCount = await Answer.countDocuments(query).exec();
...
/***** get a Course by ID  *****/
router.get("/getAnswerId/:answer", async (req, res) => {
  try {
    const answers = await answers.findOne({ _id: req.params.createevaluation }).populate(
      [
        { path: "subBoard" },
        { path: "classes" },
        { path: "subject" },
